"""
SwinUNEXTR
Author: Diedre Carmo (dscarmo)

Playing around with some ideas for text informed segmentation.
"""
...
from torch.nn import MultiheadAttention as MHAttention
...
class TextInformedRepresentationBlock(nn.Module):
    """
    Multimodal fusion block for text informed decoding

    Since this works at every UNet level, we learn to select the most relevant tokens from the text report to fuse with image features, from abstract to fine features.
    """
    def __init__(self, text_feature_size, image_feature_size, num_semantic_tokens=6):
        """
        Learn to select the most relevant text tokens for this decoding block, before late fusion with image features.

        "Semantic tokens" are a set of learnable tokens that are used to represent the most relevant text tokens for this decoding block.
        """
        super().__init__()
        assert image_feature_size % num_semantic_tokens == 0, "image_feature_size must be divisible by num_semantic_tokens"
        semantic_token_size = image_feature_size // num_semantic_tokens  # S = C / num_semantic_tokens

        # project BioBERT 768 embeddings → semantic token size
        self.text_proj = nn.Linear(text_feature_size, semantic_token_size)

        # Q-Former selecting most relevant tokens for decoding this block
        self.q_former = MHAttention(embed_dim=semantic_token_size, num_heads=1, batch_first=True)

        # Q-Former parameter, learn query tokens that are useful for segmentation
        self.q_former_param = nn.Parameter(torch.randn(1, num_semantic_tokens, semantic_token_size))

        # fuse image+text channels: (2*C --> C)
        self.fusion = nn.Conv3d(image_feature_size * 2, image_feature_size, kernel_size=1)

    def forward(self, image_features: torch.Tensor, text_features: torch.Tensor) -> torch.Tensor:
        B, seq_len, text_feature_size = text_features.shape
        B, C, D, H, W = image_features.shape
        
        # Compress to image feature size
        text_features: torch.Tensor = self.text_proj(text_features)               # [B, 768] → [B, S]

        # Select most relevant token aggregation with q-former
        query_tokens = self.q_former_param.expand(B, -1, -1)
        text_features, _ = self.q_former(query_tokens,                            # Q [B, num_semantic_tokens, S]
                                         text_features,                           # K [B, seq_len, S]
                                         text_features)                           # V [B, seq_len, S]
        text_features = text_features.reshape(B, C)                               # [B, num_semantic_tokens, S] -> [B, C]

        # Expand text_feat to match img_features shape for broadcasting
        text_features = text_features.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)   # [B, C, 1, 1, 1]
        text_features = text_features.expand_as(image_features)                   # [B, C, D, H, W]

        fused = torch.cat([image_features, text_features], dim=1)                 # [B, 2C, D, H, W]
        fused: torch.Tensor = self.fusion(fused)                                  # [B, C, D, H, W]

        return fused


class SwinUNETRTI(nn.Module):
    """
    Customization of SwinUNETR for multimodal fusion
    """
    ...
    def __init__(
        ...
        self.text_informed_representation5 = TextInformedRepresentationBlock(text_feature_size, 8 * feature_size)
        ...
        self.text_informed_representation4 = TextInformedRepresentationBlock(text_feature_size, 4 * feature_size)
        ...
        self.text_informed_representation3 = TextInformedRepresentationBlock(text_feature_size, 2 * feature_size)
        ...
        self.text_informed_representation2 = TextInformedRepresentationBlock(text_feature_size, feature_size)
        ...
        self.text_informed_representation1 = TextInformedRepresentationBlock(text_feature_size, feature_size)
    ...
    def forward(self, x_in, text_in=None):
        ...
        dec3 = self.text_informed_representation5(dec3, text_in)    

        dec2 = self.decoder4(dec3, enc3)
        dec2 = self.text_informed_representation4(dec2, text_in)

        dec1 = self.decoder3(dec2, enc2)
        dec1 = self.text_informed_representation3(dec1, text_in)

        dec0 = self.decoder2(dec1, enc1)
        dec0 = self.text_informed_representation2(dec0, text_in)

        out = self.decoder1(dec0, enc0)
        out = self.text_informed_representation1(out, text_in)
