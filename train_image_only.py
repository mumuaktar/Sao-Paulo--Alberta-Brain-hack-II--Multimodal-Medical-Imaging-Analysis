...
def train(train_loader, val_loader, model, optimizer, scheduler, config: dict):
    ...
    Args:
      ...
      Image-only training function.

      NOTE:
      This function is identical to train(), except that text features
      are ignored and the model is called in image-only mode.
    ...
    for epoch in range(start_epoch, config['max_epochs'] + 1):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(val_loader, desc="Validating", total=len(val_loader)):
                ...
                # image-only
                predictor_image_only = lambda x: model(x)

                # Create a sliding_window_inference instance with this predictor
                model_inferer_image_only = partial(
                    sliding_window_inference,
                    roi_size = [96, 96, 96],
                    sw_batch_size = 1,
                    predictor = predictor_image_only,
                    overlap = 0.5,
                )

                # Run inference
                pred_seg = model_inferer_image_only(img)
